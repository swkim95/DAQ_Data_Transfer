...
def check_transfer_location(source_path, destination_path) :
    ...
    print(f"{bcolors.INFO}[INFO]{bcolors.ENDC} Checking size of source folder : {bcolors.OKCYAN}{bcolors.BOLD}%s{bcolors.ENDC}" %(source_path) )
...
def transfer_dirs_to_hdd(source_path, destination_path) :
    ...
    copy_cmd = ("rsync -avh --itemize-changes --log-file=./Log/Copy_Log/rsync_log%s.txt --progress %s %s" %(source_name.replace("SSD", ""), source_path, destination_path))
...
if __name__ == "__main__" :
    ...
    if not len(sys.argv) == 3 : 
        ...
        print(f"{bcolors.INFO} [Usage] {bcolors.ENDC} ./Transfer_Data.sh {bcolors.UNDERLINE}<source_path>{bcolors.ENDC} {bcolors.UNDERLINE}<destination_path>{bcolors.ENDC}")
        print(f"{bcolors.INFO} [Example] {bcolors.ENDC}./Transfer_Data.sh  ~/scratch/202208TB/SSD/SSD_Run_999/ /Volumes/HDD_16TB_1/")
        sys.exit()
    ...
    if not destination_path.endswith("/") : destination_path += "/"

    if not "SSD" in source_path.split("/")[-2] :
        ...
        sys.exit()
    if not "Run" in source_path.split("/")[-2] :
        ...
        sys.exit()
    if not "HDD" in destination_path.split("/")[-2] :
    ...
    if not os.path.exists(source_path) :
        print(f"{bcolors.ERROR}[ERROR]{bcolors.ENDC} Source folder {bcolors.BOLD}\"%s\"{bcolors.ENDC} {bcolors.ERROR}does not exist{bcolors.ENDC}, please check" % (source_path) )
        sys.exit()
    if not os.path.exists(destination_path):
        print(f"{bcolors.ERROR}[ERROR]{bcolors.ENDC} Destination folder {bcolors.BOLD}\"%s\"{bcolors.ENDC} {bcolors.ERROR}does not exist{bcolors.ENDC}, please check" % (destination_path) )
        sys.exit()

    if "_copied" in source_path.split("/")[-2] :
        print(f"{bcolors.WARNING}#######################################################################################################{bcolors.ENDC}")
    ...
    print(f"{bcolors.INFO}[INFO]{bcolors.ENDC} Transferring SSD folder : {bcolors.BOLD}{bcolors.OKCYAN}%s{bcolors.ENDC} to HDD dir : {bcolors.BOLD}{bcolors.OKCYAN}%s{bcolors.ENDC}" %(source_path, destination_path))
    print(f"{bcolors.INFO}[INFO]{bcolors.ENDC} Checking folder size...{bcolors.ENDC}")

    status = check_transfer_location(source_path, destination_path)

    if (status) :
        transfer_dirs_to_hdd(source_path, destination_path)
        action_after_transfer(source_path)