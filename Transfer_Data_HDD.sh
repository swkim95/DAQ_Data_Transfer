...
if [ -z "$RUN_NUMBER" ]; then
    ...
    echo "\033[94m[Usage]\033[0m ./Transfer_Data_HDD.sh <run_number> [destination_path]"
    echo "\033[94m[Example]\033[0m ./Transfer_Data_HDD.sh 999"
    exit 1
fi

LOG_FILE="./Log_HDD/Copy_Log/Log_Run_$RUN_NUMBER.txt"
...
if [ "$DAQ_AUTOMATION" = "true" ]; then
    ...
    if [ -n "$DEST_DIR" ]; then
        python3 transfer_from_HDD_to_HDD.py ${RUN_NUMBER} ${DEST_DIR}
    else
        python3 transfer_from_HDD_to_HDD.py ${RUN_NUMBER}
    fi
else
    # Manual execution with logging
    if [ -n "$DEST_DIR" ]; then
        python3 transfer_from_HDD_to_HDD.py ${RUN_NUMBER} ${DEST_DIR} | tee -a ${LOG_FILE}
    else
        python3 transfer_from_HDD_to_HDD.py ${RUN_NUMBER} | tee -a ${LOG_FILE}
    fi
fi 
